...
const Meeting = () => {
  ...
  useEffect(() => {
    try {
      const preferences = JSON.parse(
        localStorage.getItem("meeting-preferences") || "{}"
      );
      if (preferences) {
  ...
  const setupWebRTC = async (stream: MediaStream | null) => {
    ...
    try {
      ...
      try {
        meetingDoc = await getDoc(meetingRef);
      } catch (error) {
        console.error("Error getting meeting document:", error);
        toast.error(
          "Failed to connect to meeting. Please check your internet connection."
        );
        return () => {};
      }
      ...
      const unsubscribeParticipants = onSnapshot(
        ...
        (snapshot) => {
          ...
          snapshot.forEach((doc) => {
            ...
            if (
              doc.id !== userId &&
              !peerConnections.current[doc.id] &&
              stream
            ) {
  ...
  const sendSignalingMessage = async (to: string, message: any) => {
    ...
    try {
      ...
      // Serialize the ICE candidate if present
      ...
      if (message.type === "ice-candidate" && message.candidate) {
        ...
        // Ensure SDP is also properly serialized
        serializedMessage.sdp = {
  ...
  const handleSignalingMessage = async (from: string, message: any) => {
    ...
    if (message.type === "offer") {
      // Create proper RTCSessionDescription from serialized data
      ...
      // Create proper RTCIceCandidate from serialized data
    ...
    if (!isScreenSharing) {
      try {
        ...
        if (error instanceof DOMException && error.name === "NotAllowedError") {
          toast.error(
            "Screen sharing permission denied. Please allow access to share your screen."
          );
        } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {Object.entries(remoteStreams).map(([participantId, stream]) => {
              ...
              return (
                <div
                  key={participantId}
                  className="relative bg-secondary-800 rounded-lg overflow-hidden aspect-square"
                >
                  <video
                    autoPlay
                    playsInline
            ...
            {Array.from({
              length: Math.max(0, 3 - Object.keys(remoteStreams).length - 1),
            }).map((_, i) => (
              <div
                key={i}
                className="bg-secondary-800 rounded-lg flex items-center justify-center aspect-square"
              >
      ...
      <div className="bg-secondary-800 p-4 flex justify-center">
        <div className="flex space-x-4">
          <Button
            ...
            onClick={toggleMic}
            className={
              isMicOn ? "border-white text-white hover:bg-secondary-700" : ""
            }
          >
            {isMicOn ? (
              <svg
          ...
          <Button
            ...
            className={
              isCameraOn ? "border-white text-white hover:bg-secondary-700" : ""
            }
          >
            {isCameraOn ? (
              <svg
          ...
          <Button
            ...
            className={
              !isScreenSharing
                ? "border-white text-white hover:bg-secondary-700"
                : ""
            }
          >
            <svg
        ...
        // Meeting controls section ends here
      </div>
    </div>
  );
};

// Helper function to retry Firebase operations - moved outside the component and before it
